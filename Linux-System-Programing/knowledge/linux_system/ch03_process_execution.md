...
## System Context — Process Execution in the Linux Architecture

**Where process execution fits:**
Process monitoring (`wait`), program replacement (`exec`), and task creation (`clone`) are the mechanisms by which the kernel transitions processes between program images, reclaims dead processes, and constructs lightweight tasks sharing resources. They sit at the intersection of the Process, Memory, VFS, and Signal subsystems.

```
Parent                              Kernel                         Child
  │                                                                  │
  │  fork() ─────────────────────► do_fork()  ──────────────────►  (child starts)
  │                                                                  │
  │                                task_struct:                      │  exec()
  │                                  state: TASK_RUNNING            │   ├─ VFS: open & read ELF
  │                                                                  │   ├─ MM: replace address space
  │                                                                  │   ├─ Signal: reset handlers
  │                                                                  │   └─ FDs: close FD_CLOEXEC fds
  │                                                                  │
  │  wait() ◄────────────────────── SIGCHLD  ◄────────────────────  │  exit()
  │   returns PID + status           task_struct state → ZOMBIE      │
  │   (entry freed in kernel)                                        │
```

**Subsystem interactions:**
- **Scheduler:** `wait()` blocks the parent in `TASK_INTERRUPTIBLE` sleep on a wait queue. Child `exit()` wakes the parent by sending `SIGCHLD`. With `WNOHANG`, parent polls without entering sleep.
- **VFS (Filesystem):** `execve()` opens the ELF binary via VFS, validates magic bytes / `#!` line, and calls the appropriate binary handler. FDs with `FD_CLOEXEC` are closed atomically at exec before the new program sees them.
- **Memory Manager:** `exec()` unmaps all existing VMAs and creates new ones from the ELF's program headers (text, data, BSS, stack). For `#!` scripts, the interpreter's ELF is mapped instead.
- **Signal subsystem:** `SIGCHLD` is generated by the kernel when a child changes state. On `exec()`, all installed signal handlers are reset to `SIG_DFL` (handler code no longer exists); `SIG_IGN` dispositions survive; the signal mask is preserved.
- **Namespaces (containers):** `clone()` with `CLONE_NEW*` flags creates isolated namespace objects (pid, net, mount, uts, ipc, user). The kernel reference-counts each namespace and destroys it when the last member exits.

**Failure scenarios:**
- Zombie accumulation: parent never calls `wait()` → each dead child holds a process table entry. When the table fills, `fork()` fails system-wide with `EAGAIN`. Fix: `SIGCHLD` handler that loops `waitpid(-1, NULL, WNOHANG)`.
- `SIGCHLD` race without `sigsuspend()`: child dies and delivers `SIGCHLD` between the parent's `sigprocmask(SIG_UNBLOCK)` and `pause()` → parent sleeps forever. Fix: block `SIGCHLD` before `fork()`; use `sigsuspend()` to atomically unblock and wait.
- `execvp()`/`execlp()` in set-user-ID programs: attacker places a malicious binary early in `PATH` with the same name → privilege escalation. Always use `execv()`/`execve()` with an absolute pathname in privileged code.
- `system()` in privileged programs: shell reads `PATH`, `IFS`, `LD_PRELOAD` from the environment which an attacker controls → arbitrary code execution as root. Never use `system()` in set-user-ID code — use `fork()` + `execve()` with a sanitized environment.
- Leaked FDs without `O_CLOEXEC`/`FD_CLOEXEC`: library-opened sockets and files survive into the `exec()`'d child → child inherits access to resources it should not have. This is a common security vulnerability in long-lived servers.
- `clone()` with invalid flag combinations (e.g., `CLONE_THREAD` without `CLONE_SIGHAND`) → `EINVAL`. Always check return value and consult kernel documentation for valid combinations.
- `setpgid()` race after `fork()`: child may call `exec()` before parent calls `setpgid(child_pid, ...)` → `setpgid()` fails with `EACCES`. Fix: both parent and child call `setpgid(0, child_pid)` immediately after `fork()`.

---

## Debugging

**Inspect process and FD state:**

```bash
# Find zombie processes
ps aux | grep 'Z'

# Full process info with session/group/terminal
ps -o pid,ppid,pgid,sid,tty,stat,cmd

# Trace exec, fork, clone, and wait syscalls (follow children with -f)
strace -f -e trace=execve,fork,clone,wait4,waitpid ./program

# Inspect open file descriptors
ls -la /proc/<PID>/fd        # FD numbers → file/socket/pipe paths
lsof -p <PID>                # human-readable open files

# Verify which binary a process is actually executing
readlink /proc/<PID>/exe

# Check FD_CLOEXEC status on a specific file descriptor
python3 -c "
import fcntl, sys
fd = int(sys.argv[1])
flags = fcntl.fcntl(fd, fcntl.F_GETFD)
print('CLOEXEC set:', bool(flags & fcntl.FD_CLOEXEC))
" <fd_number>

# Monitor clone() namespace creation (look for CLONE_NEWPID, CLONE_NEWNET in flags)
strace -e trace=clone -o /tmp/clone.log ./container_program
```

**Common bugs to catch in review:**

```c
/* BUG: using system() in a setuid program */
system("rm -f /tmp/lock");   /* attacker controls PATH → arbitrary command as root */

/* FIX: fork + execv with absolute path */
if (fork() == 0) {
    execv("/bin/rm", (char *[]){"/bin/rm", "-f", "/tmp/lock", NULL});
    _exit(127);
}
wait(NULL);

/* BUG: single wait() in SIGCHLD handler — misses siblings */
void handler(int sig) {
    wait(NULL);   /* only one child reaped; siblings become zombies */
}

/* FIX: loop with WNOHANG until no more children are ready */
void handler(int sig) {
    int saved = errno;
    while (waitpid(-1, NULL, WNOHANG) > 0)
        continue;
...
/* BUG: socket fd leaks into exec'd child */
int sockfd = socket(AF_INET, SOCK_STREAM, 0);
execv("/usr/bin/helper", args);   /* helper inherits sockfd */

/* FIX: set CLOEXEC at creation time */
int sockfd = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
/* or: fcntl(sockfd, F_SETFD, fcntl(sockfd, F_GETFD) | FD_CLOEXEC); */
```
...
|---|---|---|
| Shell I/O redirection (`ls > file.txt`) | `fork()` + `dup2()` + `execv()` | Fork; child closes fd 1, opens `file.txt` as fd 1, execs `ls` |
| Zombie-free server (async child reaping) | `SIGCHLD` + `waitpid(-1, WNOHANG)` | Loop in handler; collects all zombies without blocking the parent |
| Library FD hygiene | `O_CLOEXEC` / `FD_CLOEXEC` | Every FD opened internally by a library must set `CLOEXEC` to prevent leaking into child `exec()` |
| Web server worker model | `fork()` + shared listen fd | Workers share the listening socket; each accepts connections independently |
| Container creation | `clone(CLONE_NEWPID\|CLONE_NEWNET\|CLONE_NEWNS\|...)` | Docker, LXC, systemd-nspawn use `clone()` namespaces for isolation |
| Resource accounting after wait | `getrusage(RUSAGE_CHILDREN)` | Accumulated after each `wait()` — measures total CPU used by all reaped children |
| Securely run external command | `fork()` + sanitize env + `execve()` with absolute path | Wipe `environ`, set only known-safe variables, exec with absolute pathname — never `system()` |
| REPL / scripting language | `fork()` + `execvp()` + `waitpid()` | Each statement executes in a child; parent always available for the next input |
